...
```
the data points appear to be reletivly grouped across the 24 species plotted
```
...
```{r}
# Code here
ggplot(surveys_filtered, aes(x = hindfoot_length, y = weight, color = species_id)) + geom_point(size = 0.5) + geom_smooth(method = "lm", color = "navy", size = 0.5, fill = "deeppink4") + labs(x = "Hindfoot Length (mm)", y = "Species Weight (g)", title = "Linear Regression to predict species weight by Hindfoot Length") 
```
...
residuals test as non normal because its comparing all three species' residuals in this test
```
...
ggplot(surveys_filtered, aes(x = hindfoot_length, y = weight, color = species_id)) + geom_point(size = 0.5) + geom_smooth(method = "lm", color = "navy", size = 0.5, fill = "deeppink4") + labs(x = "Hindfoot Length (mm)", y = "Species Weight (g)", title = "Linear Regression to predict species weight by Hindfoot Length") + facet_wrap(facets = vars(species_id))
```
...
```{r}
 ...
 ggplot(surveys_filtered_sex, aes(x = sex , y = weight)) 
         geom_point()
#AMW: this was a blank plot for me? 3/5
```
...
# AMW: The below didn't work for me? 
...
```
go to the git tab, click commit, add comment, close window.
```

2) Push your changes to github (10 pts)

```
go to the git tab, click the green push button, put in username and password.  
```
...
#AMW: I didn't see this anywhere?