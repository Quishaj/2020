...
leaflet(mega_df) %>% 
  addTiles() %>% 
  addCircleMarkers(~decimalLongitude, ~decimalLatitude, color = ~pal(scientificName))  %>%
...
```
You'll notice that a couple of the taxa have undergone taxonomic revision. We can replace the out of date names like so:

```{r}

subset_data$genusSpecies[subset_data$genusSpecies == "Atta_colombica"] <- "Atta_columbica"
subset_data$genusSpecies[subset_data$genusSpecies == "Atta_mexicana"] <- "Atya_scabra"
```

This isn't crucial right now, but later in the lesson a mismatch of tip labels and GBIF coordinates will be fatal.
...
tree <- rotl::tol_induced_subtree(reconciled_names$ott_id, label="name")
```
